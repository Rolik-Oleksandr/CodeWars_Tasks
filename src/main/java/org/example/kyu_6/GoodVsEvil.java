...
public class GoodVsEvil {
  public static String battle(String goodAmounts, String evilAmounts) {
    int[] goodCounts = Stream.of(goodAmounts.split(" ")).mapToInt(Integer::parseInt).toArray();
    int[] evilCounts = Stream.of(evilAmounts.split(" ")).mapToInt(Integer::parseInt).toArray();

    int goodWorth = goodCounts[0] + goodCounts[1] * 2 + goodCounts[2] * 3 + goodCounts[3] * 3 + goodCounts[4] * 4 + goodCounts[5] * 10;
    int evilWorth = evilCounts[0] + evilCounts[1] * 2 + evilCounts[2] * 2 + evilCounts[3] * 2 + evilCounts[4] * 3 + evilCounts[5] * 5 + evilCounts[6] * 10;
